...
namespace Librarian2._0._1
{
    public partial class Form_ReturnBook : Form
    {
        ...
        public Form_ReturnBook(string UserName, string UserID)
        {
            InitializeComponent();
        ...
        private void BTN_ReturnBook_Click(object sender, EventArgs e)
        {
            using (SqlConnection cn = new SqlConnection(LocalDB_conn))
            {
                ...
                sqlUpdate = $"UPDATE books SET state = '未借出' ,lastReturnDate = CONVERT(varchar(100), GETDATE(), 23) FROM books WHERE booksID like '{TB_BookID.Text}'";
                ...
                try
                {
                    if (dataReader_search["state"].ToString() == "借出中")
                    {
                        ...
                        sql_log_Insert = $"INSERT INTO logs VALUES ('{TB_BookID.Text}','{LB_BookName.Text}','{LB_UserID.Text}','{LB_UserName.Text}',NULL,CONVERT(varchar(100), GETDATE(), 120),NULL)";
